...
int main(int argc, char **argv) {
	struct mqtt_session session;
	int connect_flag = 0;
	cmqtt_init(&session, "192.168.205.225", 1883, "1111111");
				...
				if (connect_flag == 0) {
					if (MQTTParseMessageType(session.inpacket.data) == MQTT_MSG_CONNACK) {
						if (session.inpacket.data[3] == 0x00) {
							...
							int dlen = 1;
							const char *msg =
									"04 05 31 31 30 38 33 EE EE 00 78 02 10 01 AF B2 C3 AE EF BD 71 82 C4 17 D5 F6 60 3B CA BE 8F C0 C0 63 1E FE 51 50 9F 7D 36 65 8D A6 A6 B9 2A D1 " \
									"CA 4B C0 7F 00 E6 74 34 5B 2C 36 D9 D5 F5 58 4E 56 D5 83 94 51 B6 E0 CC D7 06 B5 09 D4 34 64 FD A7 8B 5C 66 03 30 DB CF 75 C6 3A EA 51 5E "\
									"B6 E7 85 67 BC 8A 81 1C A1 28 CB 56 C6 0B 4C 02 6A A8 9F D4 12 65 6F 90 52 FC 6F 9F 62 F3 C8 5D E6";
							dlen = tools_hexstring_2_data(msg, data, sizeof(data));
							mqtt_publish_data(&session.broker, "HK/67bdf38930b840a6bd65", data,
									dlen, 0,1, NULL);
						} else {
							//connect error
							break;
