...
uint16_t mqtt_parse_rem_len(const uint8_t* buf) {
	...
	do {
		digit = *buf;
		value += (digit & 127) * multiplier; // (d&0x7F)*m;
		multiplier *= 128;  //<<7
		buf++;
	} while ((digit & 128) != 0);
...
int mqtt_publish_with_qos(mqtt_broker_handle_t* broker, const char* topic, const char* msg,
		uint8_t retain, uint8_t qos, uint16_t* message_id) {
...
int mqtt_remainlen2bytes(size_t len, unsigned char* buff) {
	int siz = 0;
	unsigned char d = 0;
	do {
		d = len % 128;
...
int mqtt_publish_data(mqtt_broker_handle_t* broker, const char* topic, const void* data, int dlen,
		uint8_t retain, uint8_t qos, uint16_t* message_id) {
	uint16_t topiclen = strlen(topic);
...
//	4 个字节时,从 2097152(0x80,0x80,0x80,0x01)到 268435455(0xFF,0xFF,0xFF,0x7F)
	...
	// need up to two bytes of length (handles up to 16,383 (almost 16k) sized message)
	//uint8_t fixedHeaderSize = 2; // Default size = one byte Message Type + one byte Remaining Length
	uint16_t remainLen = sizeof(var_header) + dlen;
	uint8_t fixed_header[5]; //Max:5Byte
	uint8_t fixedHeaderSize = 1;

	//Bit: 7-4(Type) 3(DUP) 2-1(QOS) 0 RETAIN
	// Message Type, DUP flag, QoS level, Retain
	fixed_header[0] = MQTT_MSG_PUBLISH | qos_flag;
	if (retain) {
		fixed_header[0] |= MQTT_RETAIN_FLAG;
	...
	// Remaining Length
	fixedHeaderSize += mqtt_remainlen2bytes(remainLen, &fixed_header[1]);

	uint8_t *packet = NULL;
	uint32_t packlen = fixedHeaderSize + sizeof(var_header) + dlen;
	packet = (uint8_t*) malloc(packlen);
	///uint8_t packet[sizeof(fixed_header) + sizeof(var_header) + msglen];
	...
	memcpy(packet + fixedHeaderSize + sizeof(var_header), data, dlen);

	// Send the packet
	if (broker->send(broker->socket_info, packet, packlen) < packlen) {
		free(packet);
		return -1;
	}
	free(packet);
