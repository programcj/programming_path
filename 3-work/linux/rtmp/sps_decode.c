...
void h264_sps_parse(unsigned char *buf, int nLen, int *pwidth, int *pheight)
{
	...
	*pheight = (pic_height_in_map_units_minus1 + 1) * 16; //* (2 - frame_mbs_only_flag)
	...
	if (frame_cropping_flag)
	{
		...
		*pwidth = *pwidth - 2 * (frame_crop_right_offset + frame_crop_left_offset);
		*pheight = *pheight - 2 * (frame_crop_top_offset + frame_crop_bottom_offset);
	}
	int vui_parameter_present_flag = u(1, buf, &StartBit);
	if (vui_parameter_present_flag)
	{
