...
int ffmpegInOpen(struct ffmpegIn *in, const char *url)
{
	...
	memset(in->errstr, 0, 64);
	if (strncasecmp("rtsp://", url, strlen("rtsp://")) == 0)
	{
		av_dict_set(&opts, "rtsp_transport", "tcp", 0); //设置tcp or udp，默认一般优先tcp再尝试udp
		//av_dict_set(&opts, "buffer_size", "1048576", 0); //1M(https://www.bejson.com/convert/filesize/)
		av_dict_set(&opts, "buffer_size", "1572864", 0); //1.5M(https://www.bejson.com/convert/filesize/)		
		//av_dict_set(&opts, "buffer_size", "2097152", 0); //2M(https://www.bejson.com/convert/filesize/)

		av_dict_set(&opts, "max_delay", "500000", 0);
		//av_dict_set(&opts, "stimeout", "3000000", 0); //3S
		av_dict_set(&opts, "stimeout", "5000000", 0); //设置超时  -stimeout的单位是us微妙
		//av_dict_set(&opts, "fflags", "nobuffer", 0);
		//通过这两个参数决定 avformat_find_stream_info 时间长度
		//
		//std::string probesize = ConfigIni::Instance().get(CFG_NAME_FF_probesize);
		//std::string analyzeduration = ConfigIni::Instance().get(CFG_NAME_FF_analyzeduration);

		//if(probesize.size()>0)
		//	av_dict_set(&opts, "probesize", probesize.c_str(), 0);

		//if(analyzeduration.size()>0)
	...
	if ((ret = avformat_open_input(&in->fmt_ctx, url, NULL, opts ? &opts : NULL)) < 0)
	{
		...
		av_make_error_string(in->errstr, AV_ERROR_MAX_STRING_SIZE, ret);
		goto _errret;
	}
	if (opts)
	{
	...
	in->fmt_ctx->max_analyze_duration = 1 * AV_TIME_BASE;
	...
	strncpy(in->video_codec_name, avcodec_get_name(vstream->codecpar->codec_id), 32 - 1);
	...
	//	tbc  表示视频流codec timebase ，对于264码流该参数通过解析sps间接获取（通过sps获取帧率）。
	{
		int fps = vstream->avg_frame_rate.den && vstream->avg_frame_rate.num;
		int tbr = vstream->r_frame_rate.den && vstream->r_frame_rate.num;
		int tbn = vstream->time_base.den && vstream->time_base.num;
		int tbc = vstream->codec->time_base.den && vstream->codec->time_base.num;

		in->fps = 0;
		if (fps)
			in->fps = (int)av_q2d(vstream->avg_frame_rate); // , tbr || tbn || tbc ? "fps, " : "fps");
	}
	printf("%s,%dx%d,%dfps,%s, duration:%ld, start:%ld\n",
				url, in->width, in->height, in->fps, in->video_codec_name,
				in->fmt_ctx->duration,
				in->fmt_ctx->start_time);
	printf("video.stime:%ld \n", in->vstream->start_time);
	in->saw_first_ts = 0;
	in->dts = AV_NOPTS_VALUE;
	in->pts = AV_NOPTS_VALUE;
	in->next_dts = AV_NOPTS_VALUE;
	in->next_pts = AV_NOPTS_VALUE;
	return 0;
	...
	if (ret == AVERROR_HTTP_BAD_REQUEST)
		ret = 400; //FFERRTAG(0xF8,'4','0','0')
	if (ret == AVERROR_HTTP_UNAUTHORIZED)
		ret = 401; //FFERRTAG(0xF8,'4','0','1')
	if (ret == AVERROR_HTTP_FORBIDDEN)
		ret = 403; //FFERRTAG(0xF8,'4','0','3')
	if (ret == AVERROR_HTTP_NOT_FOUND)
		ret = 404; //FFERRTAG(0xF8,'4','0','4')
	if (ret == AVERROR_HTTP_OTHER_4XX)
		ret = 400; //FFERRTAG(0xF8,'4','X','X')
	if (ret == AVERROR_HTTP_SERVER_ERROR)
		ret = 500; //FFERRTAG(0xF8,'5','X','X')
	return ret;
}
...
int ffmpegDecOpen(struct ffmpegDec *dec, struct ffmpegIn *in)
{
	...
	if (vstream->codecpar->codec_id == AV_CODEC_ID_H264)
	{
		...
		if (dec->pCodec != NULL)
		{
			printf("使用Nvidia CUVID H264 decoder\n");
		}
	}
	else if (vstream->codecpar->codec_id == AV_CODEC_ID_HEVC)
	{
		...
		if (dec->pCodec != NULL)
		{
			printf("使用Nvidia CUVID HEVC decoder\n");
		}
	}
#endif
	if (dec->pCodec == NULL)
...
int ffmpegDecRead(struct ffmpegDec *fdec, AVFrame *avframe)
{
	...
	ret = avcodec_receive_frame(fdec->pCodecCtx, avframe); //内部会av_frame_unref,av_pix_fmt_desc_get
	if (ret != 0)
	{
			...
			const char *types)
{
	...
	memset(fenc->errstr, 0, 64);
	ret = avformat_alloc_output_context2(&ofmt_ctx, NULL, types, out_url);
	if (ret < 0)
	{
		printf("avformat_alloc_output_context2 err\n");
		av_make_error_string(fenc->errstr, AV_ERROR_MAX_STRING_SIZE, ret);
		goto err;
	}
	//avformat_query_codec(oc->oformat, oc->oformat->video_codec, 0)
	...
	///for (int i = 0; i < pFormat->nb_streams; i++)
	{
		...
		if (!out_stream)
		{
			...
			strcpy(fenc->errstr, "new stream error");
			goto err;
		}
		...
		{
			printf("拷贝视频code失败\n");
			strcpy(fenc->errstr, "param copy error");
			goto err;
		}
	...
	if (!(ofmt_ctx->flags & AVFMT_NOFILE))
	{
		...
		//ret = avio_open2(&ofmt_ctx->pb, out_url, AVIO_FLAG_WRITE, &int_cb, NULL);
		if (ret < 0)
		{
			av_make_error_string(fenc->errstr, AV_ERROR_MAX_STRING_SIZE, ret);
			printf("Could not open output URL '%s'(%s), %s\n", out_url, types, fenc->errstr);
			goto err;
		}
	}

	ret = avformat_write_header(ofmt_ctx, NULL); //写文件头
	if (ret < 0)
	{
		strcpy(fenc->errstr, "write header error");
		ret = -1;
		goto err;
	}
...
int ffmpegEncOpen2(struct ffmpegEnc *fenc, AVStream *in_stream, const char *out_url)
{
	...
	return ffmpegEncOpen(fenc, in_stream->codecpar->codec_id, in_stream->codecpar, out_url, NULL);
}
...
{
	fenc->src_time_base = in_stream->time_base;
	return ffmpegEncOpen(fenc, in_stream->codecpar->codec_id, in_stream->codecpar, out_url, types);
}
...
void _AVFormatContextClose(AVFormatContext *format_ctx)
{
	...
	if (format_ctx)
	{
		if ((ret = av_write_trailer(format_ctx)) < 0)
			av_log(NULL, AV_LOG_ERROR, "Error writing trailer of %s\n", format_ctx->url);
	}
	...
	{
		ret = avio_closep(&format_ctx->pb);
		printf("avio_close, ret=%d\n", ret);
	}
