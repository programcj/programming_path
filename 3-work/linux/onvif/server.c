...
int digest_server_parse(digest_t *digest, const char *digest_string)
{
...
int digest_server_generate_nonce(digest_t *digest)
{
	// digest_s *dig = (digest_s *) digest;
 ...
 */
size_t digest_server_generate_header(digest_t *digest, char *result, size_t max_length)
{
	digest_s *dig = (digest_s *) digest;
