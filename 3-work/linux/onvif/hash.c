 ...
 * 32 characters long.
 */
static void _get_md5(const char *string, char *result)
{
	int i = 0;
 ...
 * All other arguments should be null terminated strings.
 */
void hash_generate_response_auth(char *result, const char *ha1, const char *nonce, unsigned int nc, char *cnonce, const char *qop, const char *ha2)
{
	char raw[512];
	sprintf(raw, "%s:%s:%08x:%s:%s:%s", ha1, nonce, nc, cnonce, qop, ha2);
	_get_md5(raw, result);
}
