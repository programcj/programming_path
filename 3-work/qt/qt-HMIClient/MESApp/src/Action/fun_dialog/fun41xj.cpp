#include "fun41xj.h"
#include "ui_fun41xj.h"
#include "../tooltextinputdialog.h"

Fun41xj::Fun41xj(ButtonStatus *btStatus, QWidget *parent) :
		QDialog(parent, Qt::FramelessWindowHint), ui(new Ui::Fun41xj)
{
	ui->setupUi(this);
...
//保存
void Fun41xj::on_btSave_clicked()
{
	QList<int> valueList;
	{
		int sum = 0;
	...
	info.setCardType(1); //	1	HEX	刷卡原因编号:巡机、打磨等 1代表巡机  2代表打磨
	for (int i = 0; i < m_order.orderBoyList.size(); i++)
	{
		...
		//更改数据库中的值
		m_order.orderBoyList[i].OnAddCurClassInspectNo(valueList[i]);
	}
	//再保存到数据库
	Notebook("巡机", info).insert();
	ShowOrderInfo();
...
//结束
void Fun41xj::on_btExit_clicked()
{
	//需要刷卡
	close();
}

void Fun41xj::ShowOrderInfo()
{
...
void Fun41xj::on_tableWidgetValue_itemClicked(QTableWidgetItem *item)
{
	ToolTextInputDialog dialog(0, ToolTextInputDialog::typeNumber, "", "请录入巡机数量");
