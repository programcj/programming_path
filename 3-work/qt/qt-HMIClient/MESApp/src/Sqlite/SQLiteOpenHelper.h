...
#ifndef SQLITEOPENHELPER_H_
#define SQLITEOPENHELPER_H_

#include <QtSql>
#include <QString>
#include "Entity/absentity.h"
...
public:
	...
	bool exec(QSqlQuery &query, const QString &file = "",
	 ...
	 * limit: 用于选择的
	 */
	bool selectItem(entity::AbsEntity &value, const QString &columns = "*",
			const QString &selection = "", const QString &orderBy = "",
			const QString &limit = "0,1") const
	{
		QSqlQuery query(m_db);
		QString sql;
		sql = "SELECT " + columns + " from " + value.GetTableName();
		if (selection.length() > 0)
			sql += " WHERE " + selection;
		if (orderBy.length() > 0)
			sql += " order by " + orderBy;
